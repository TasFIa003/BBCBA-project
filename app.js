...
const MembershipUpdate=new mongoose.model("MembershipUpdate",membershipSchema);
...
app.post("/",function(req,res){
    ...
    Life.findOne({mail:email},function(err,result){
      //console.log(email);
      if(err){
        //console.log(err);
...
app.post("/userpanel",upload,function(req,res){

    const upusers=new MembershipUpdate({
      name:req.body.name,
      ...
      upusers.save(function(err){
        if(!err){
          //console.log("succesful");

          res.send("Thank you for your information.Your information will be updated by the approval of admin . For any query you can sent email to Admin. Thank You!");
...
app.get("/checkupdate",function(req,res){
  ...
  MembershipUpdate.find({},function(err,membs){

      res.render("checkupdate",{
        membs:membs
      });
...
app.get("/cmudelete/:id",function(req,res){

      MembershipUpdate.findByIdAndDelete({_id:req.params.id},function(err){
      if(err){
            //console.log(err);
        }else{
            res.send("Deleted succesfully");
